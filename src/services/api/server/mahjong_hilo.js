...
// const BASE_URL = `http://localhost:3006/mahjong-hilo`
// axios.get(BASE_URL + `/donut/api/v1/game/ghost-quest/character/${user}/${id}`)
...
//   return await GetUserWalletBalance()
// }

// export const MJ_DEPOSIT_TOKEN = ({ username, amount }) => {
//   const headers = getHeaderParams()
//   const data = { username, amount }
//   return axios.post(`${MJ_URL}/deposit-token`, data, { headers })
// }
...
export const MJ_ACTION_END = () => {
  const headers = getHeaderParams()
  return axios.get(`${MJ_URL}/quit`, { headers })
}

export const MJ_ACTION_RESET_GAME = () => {
  const headers = getHeaderParams()
  return axios.get(`${MJ_URL}/reset`, { headers })
}
...
export const MJ_HISTORY = (id) => {
  const headers = getHeaderParams()
  // const data = { id }
  return axios.get(`${BASE_URL}/game/all/history/${game_id}/${id}`, { headers })
}
