...
import { BtnSolid } from "screens/MahjongGameplay/Btn";
...
const MyData = ({ onHide, playerID, winRate, maxPayout, consecutiveHiLo, name }: Props) => {
        ...
        <div className={styles.content}>
          <div className={styles.profilePicContainer}>
            ...
            <div className={styles.nameContainer}>
              <div>{name}</div>
              <div>{translate("mj.my_data.id")}: {playerID}</div>
            </div>
          </div>

          <div className={styles.btnWrapper}>
            {showDetails.mahjong && <MahjongWins maxPayout={maxPayout} consHiLo={consecutiveHiLo} winRate={winRate} />}
            {showDetails.hilo && (
              <HiLoWins
            ...
            <BtnSolid
              isDisabled={showDetails.hilo}
              containerClass={styles.btn}
              label={translate("mj.my_data.mahjong.button") as unknown as string}
              onClick={onToggleMahjong}
              labelClass={styles.btnLabel}
            />
            <BtnSolid
              isDisabled={showDetails.mahjong}
              containerClass={styles.btn}
              label={translate("mj.my_data.hilo.button") as unknown as string}
